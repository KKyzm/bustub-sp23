...
LRUKReplacer::LRUKReplacer(size_t num_frames, size_t k)
    : replacer_size_(num_frames), k_(k), begin_timestamp_(std::chrono::high_resolution_clock::now()) {}

auto LRUKReplacer::Evict(frame_id_t *frame_id) -> bool {
  auto guard = std::scoped_lock(lru_k_replacer_latch_);
...
void LRUKReplacer::RecordAccess(frame_id_t frame_id, [[maybe_unused]] AccessType access_type) {
  ...
  // trim history to only record the most recent k_ timestamps
...
void LRUKReplacer::Remove(frame_id_t frame_id) {
  ...
  const auto evictable = node_store_.at(frame_id).is_evictable_;
  if (!evictable) {
    throw Exception("Try to remove an unevictable frame");
  }
...
void LRUKReplacer::EnsureFrameIdValid(frame_id_t frame_id) const {
  if (static_cast<size_t>(frame_id) >= replacer_size_) {
    throw Exception("Invalid frame id");
  }
}

auto LRUKReplacer::HaveFrameId(frame_id_t frame_id) const -> bool {
  return node_store_.find(frame_id) != node_store_.end();
}

auto LRUKReplacer::CurrentTimeStamp() const -> size_t {
