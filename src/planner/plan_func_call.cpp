...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower" or func_name == "upper") {
    if (args.size() != 1) {
      throw Exception(fmt::format("func call {} #args is {}", func_name, args.size()));
    }
    auto exp_type = StringExpressionType::Lower;
    if (func_name == "upper") {
      exp_type = StringExpressionType::Upper;
    }
    return std::make_shared<StringExpression>(args[0], exp_type);
  }
